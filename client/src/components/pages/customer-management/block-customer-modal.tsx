...
import { blockcustomers } from "../../../api/endpoints/customer-management";
...
export default function BlockStudentModal({open,setOpen,updated,setUpdated,id}:ModalProps) {
  ...
  const handleBlock = async (customerId: string, reason: string) => {
    try {
      const response = await blockcustomers(customerId, reason);
