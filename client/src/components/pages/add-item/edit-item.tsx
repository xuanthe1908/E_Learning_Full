...
import { FormValuesItem } from "../../../types/item";
...
import { getQuizzesByItem } from "../../../api/endpoints/product/quiz";
import { Question } from "../../../api/types/apiResponses/api-response-quizzes";
import { editItem } from "../../../api/endpoints/product/item";
...
const EditItemForm: React.FC = () => {
  ...
  const [itemInfo,setItemInfo] = useState<ApiResponseItem|null>()
  ...
  const handleSubmit = async (
    item: FormValuesItem,
    { resetForm }: FormikHelpers<FormValuesItem>
  ) => {
    try {
      setIsUploading(true);
      const formData = new FormData();
      lessonVideo && formData.append("media", lessonVideo, "itemVideo");
      materialFile && formData.append("media", materialFile, "materialFile");
      Object.keys(item).forEach((key) => {
        if (key === "questions") {
          const questionsJSON = JSON.stringify(item[key]);
      ...
      const response = await editItem(itemId ?? "", formData);
  ...
  const fetchQuiz = async ()=>{
    try {
      const response = await getQuizzesByItem(itemId??"")
  ...
  useEffect(()=>{
   if(itemInfo){
    initialValues.title=itemInfo.title
    initialValues.description=itemInfo.description
    initialValues.contents=itemInfo.contents.join(' ')
    initialValues.about=itemInfo.about
