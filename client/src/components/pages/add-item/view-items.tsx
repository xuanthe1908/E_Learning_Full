...
import { ApiResponseItems } from "../../../api/types/apiResponses/api-response-sellers";
import AddItemForm from "./add-items-form";
...
const ViewItems: React.FC = () => {
  const [items, setItems] = useState<ApiResponseItems[] | null>(null);
  const [formVisible, setFormVisible] = useState<boolean>(false);
  const params = useParams<{ productId: string }>();
  const navigate = useNavigate();
  const productId = params.productId;

  console.log('🔍 ViewItems - Raw params:', params);
  console.log('🔍 ViewItems - Extracted productId:', productId);
  ...
  const fetchData = async (productId: string) => {
    ...
    try {
      console.log('📤 Fetching items for product:', productId);
        ...
        <div className='mb-8 flex items-center justify-between gap-8'>
          ...
          <div className='flex shrink-0 flex-col gap-2 sm:flex-row'>
            ...
            >
              ...
              {formVisible ? "View items" : "Add items"}
            </Button>
          </div>
        </div>
                    ...
                    return (
                      <li key={_id} className={`flex ${classes}`}>
                        ...
                        <div className='flex items-center mr-6 gap-2'>
                          ...
                          <Link to={`/sellers/view-items/${productId}/edit-item/${_id}`}>
                            <IconButton variant='text' color='blue-gray'>
                              <PencilIcon className='h-4 w-4' />
                            </IconButton>
                            </Link>
                          </Tooltip>
                          <Tooltip content='Disable item'>
                            <IconButton variant='text' color='blue-gray'>
