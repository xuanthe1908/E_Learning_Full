...
import TrendingCardShimmer from "components/shimmer/shimmer-trending-product";
...
const CustomerHomePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    isLoggedIn && user === "customer" && fetchRecommendedproducts();
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/20 to-purple-50/20">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 justify-items-center">
          {trendingproducts?.slice(0, cardsToShow).map((product) => (
            <TrendingCard key={product._id} productInfo={product} />
          ))}
        </div>
        
        {trendingproducts && trendingproducts.length > cardsToShow && (
      ...
      {recommendedproducts && recommendedproducts.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 bg-white/60 backdrop-blur-sm rounded-3xl mx-4 mb-8 shadow-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 justify-items-center">
            {recommendedproducts?.slice(0, cardsToShow).map((product, index) => (
              <RecommendedCard key={index} productInfo={product} />
            ))}
          </div>
          
          {recommendedproducts.length > cardsToShow && (
            <div className="text-center mt-12">
              <button
