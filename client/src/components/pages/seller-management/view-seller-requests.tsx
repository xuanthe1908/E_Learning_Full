...
const ViewSellerRequests: React.FC = () => {
  ...
  const handleApiCall = async () => {
    ...
    // ✅ Production Mode
    try {
      const response = await getAllSellerRequests();
  ...
  return (
    <ul role='list' className=' divide-gray-100  '>
      {requests?.map((person: SellerApiResponse) => (
        <Link
          to={`/admin/sellers/requests/${person._id}`}
          key={person?._id}
        >
...
export default ViewSellerRequests;
