...
import { acceptSellerRequest } from "../../../api/endpoints/seller-management";
...
import { SellerApiResponse } from "../../../api/types/apiResponses/api-response-sellers";
  ...
  const ViewMoreSellerRequest: React.FC = () => {
  const { id } = useParams();
  const [seller, setSeller] = useState<SellerApiResponse>();
  const [open, setOpen] = useState<boolean>(false)
  const fetchInfo = async () => {
    try {
      const response = id && await getIndividualSellers(id);
      response && setSeller(response.data.data);
    } catch (error) {
      // Handle error
    }
  };
  ...
  const acceptRequest = async () => {
    try {
      const response = await acceptSellerRequest(_id);
          ...
          {certificates.length > 0 && (
            <div className="px-4 py-4 sm:py-6">
              ...
              <div className="grid grid-cols-3 gap-4 mt-2 justify-between">
                {certificates.map((certificate: any, index: number) => <div
                        key={index}
...
export default ViewMoreSellerRequest;
