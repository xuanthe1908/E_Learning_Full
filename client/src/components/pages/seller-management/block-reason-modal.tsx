...
import { blockSellers } from "../../../api/endpoints/seller-management";
...
export default function BlockReasonModal({open,setOpen,updated,setUpdated,id}:ModalProps) {
  ...
  const handleBlock = async (sellerId: string, reason: string) => {
    try {
      const response = await blockSellers(sellerId, reason);
