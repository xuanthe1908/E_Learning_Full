...
import { getIndividualSellers } from "../../../api/endpoints/seller-management";
import { SellerApiResponse } from "../../../api/types/apiResponses/api-response-sellers";
import { toast } from "react-toastify";
import { Avatar } from "@material-tailwind/react";
import Viewsellershimmer from "components/shimmer/view-seller-shimmer";

type Props = {};

const Viewseller: React.FC = (props: Props) => {
  const [seller, setSeller] = useState<SellerApiResponse | null>(
    null
  );
  ...
  const fetchSeller = async () => {
    try {
      setIsLoading(true);
      const response = await getIndividualSellers(tutorId ?? "null");
      ...
      <div className='flex justify-center items-center '>
        <div className='w-11/12 md:w-8/12 p-5 -mt-10 md:-mt-20 bg-white shadow-xl rounded-xl h border border-gray-300 flex flex-col md:flex-row'>
          ...
          <div className='w-full md:w-3/4 p-4 md:pt-14 text-customFontColorBlack  md:pr-14 pb-14'>
            ...
            <p className='text-base'>{seller?.about}</p>

            <div className='mt-8'>
              <h2 className='text-xl font-semibold mb-2'>Skills</h2>
              <p className='text-base'>{seller?.skills}</p>
            </div>

            <div className='mt-8'>
              <h2 className='text-xl font-semibold mb-2'>Qualification</h2>
              <p className='text-base'>{seller?.qualification}</p>
            </div>

            <div className='mt-8'>
              <h2 className='text-xl font-semibold mb-2'>Experience</h2>
              <p className='text-base'>{seller?.experience}</p>
            </div>
          </div>
        </div>
      </div>
