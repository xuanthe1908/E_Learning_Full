...
import {
  ...
  deleteChat,
  analyzeQuery
} from '../api/endpoints/shop/shopEndpoints';
// eslint-disable-next-line @typescript-eslint/no-unused-vars
import { ShopResponse, ChatMessage, CreateChatRequest, SendMessageRequest } from '../api/services/shop/shopService';

export const useShop = () => {
  const [chats, setChats] = useState<ShopResponse[]>([]);
  const [currentChat, setCurrentChat] = useState<ShopResponse | null>(null);
  ...
  const createNewChat = useCallback(async (data: CreateChatRequest) => {
    if (!isLoggedIn) {
      toast.error('Vui lòng đăng nhập để sử dụng Shop');
      return;
    }
